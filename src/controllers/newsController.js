...
const NewsController = {
	...
	addNews: async (request, h) => {
		try {
			const { newsTitle, newsAuthor, date, description, picture, content } =
				request.payload;
			...
			return h
				.response({ message: "News added successfully", news: newNews })
				.code(201);
		} catch (err) {
	...
	updateNews: async (request, h) => {
		try {
			...
			return h
				.response({ message: "News updated successfully", news })
				.code(200);
		} catch (err) {
